...
async function generateInstagramImage(
  ...
  try {
    ...
    const fileName = `${sanitizedHeadline.replace(/\s/g, '-')}-${Date.now()}.jpg`;
    ...
    while (!uploadSuccess && retryCount < maxRetries) {
      try {
        ...
        if (retryCount > 0) {
          ...
          console.log(`Retrying upload in ${delay}ms (attempt ${retryCount + 1}/${maxRetries})`);
          await new Promise(resolve => setTimeout(resolve, delay));
        } else {
          await new Promise(resolve => setTimeout(resolve, 500));
        }
        
        console.log(`Attempting upload to Supabase Storage (attempt ${retryCount + 1}/${maxRetries})`);
        
        const { error } = await supabase.storage
          .from('news-images')
          .upload(fileName, imageWithText, {
        ...
        console.error(`Upload attempt ${retryCount + 1} threw error:`, err);
    ...
    if (!uploadSuccess) {
      ...
      console.log('Falling back to placeholder image due to upload failure');
      return { instagram: 'https://placehold.co/1080x1080?text=Upload+Failed' };
    }
    ...
    console.log('Successfully generated image with text overlay and uploaded to Supabase Storage');
    return { instagram: urlData.publicUrl };
  } catch (error) {
...
export async function GET() {
  try {
    ...
    for (const post of selectedPosts) {
      ...
      // Add delay between posts to prevent connection issues
      ...
      if (imageUrl && isValidUrl(imageUrl)) {
        try {
          ...
          if (platformImages.instagram && platformImages.instagram.startsWith('https')) {
            ...
            isValidImage = true;
            console.log('Successfully generated image and uploaded to Supabase Storage');
          } else {
            console.warn(`Skipping post due to invalid image generated by Supabase Storage`);
          }
        } catch (error) {
