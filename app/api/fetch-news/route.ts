...
async function generateInstagramImage(
  ...
  try {
    ...
    const base64Image = `data:image/jpeg;base64,${imageWithText.toString('base64')}`;
    
    return { instagram: base64Image };
  } catch (error) {
    ...
    for (const post of selectedPosts) {
      ...
      if (imageUrl && isValidUrl(imageUrl)) {
        try {
          ...
          if (platformImages.instagram && platformImages.instagram.startsWith('data:image')) {
            postPayload = { instagram: { image_url: platformImages.instagram, caption: presetCaption } };
            isValidImage = true;
            console.log('Successfully generated image with sharp');
          } else {
            console.warn(`Skipping post due to invalid image generated by sharp`);
          }
        } catch (error) {
